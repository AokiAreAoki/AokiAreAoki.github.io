...
declare namespace Flex {
	...
	interface Props extends StyledProps, PropsWithChildren {
		style?: Omit<React.CSSProperties, FlexProperties>;
	}
}

function shorthandValue<V>(cssRule: string, value: V | boolean | undefined, shorthand: V) {
	if (!value) return "";

	const ruleValue = value === true ? shorthand : value;

	return `${cssRule}: ${ruleValue};`;
}

const StyledFlex = styled.div<Flex.StyledProps>`
	display: flex;
...
const Flex: FC<Flex.Props> = ({ children, style, ...props }) => (
